...
module.exports = (
  ...
  const generateImagesAndUpdateNode = async function (
    ...
    let title = nodeTitle.trim().replace(/=\d{2,4}x\d{2,4}|=\d{2,4}/gi, ' ').trim()
    ...
    if (width) {
      ...
      ratio = height ? height + "px" : (1 / fluidResult.aspectRatio) * maxWidth + 'px'
    }
  ...
  <span
    ...
    style="width: ${width}px; padding-bottom: ${ratio}; position: relative; bottom: 0; left: 0;${bgImage} display: block;"
  ></span>
  ${imageTag}
  `.trim()
